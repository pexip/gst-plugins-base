...
GST_START_TEST (test_rtp_audio_level_meta_set_get)
{
  GstBuffer *in;
  GstBuffer *rtp;
  GstBuffer *out;
  ...
  fail_unless (gst_rtp_audio_level_meta_add_one_byte_ext (in_meta, &rtp_buf,
          id));
...
rtp_meta_suite (void)
{
  ...
  tcase_add_loop_test (tc_chain, test_rtp_audio_level_meta_set_get, 1, 15);
