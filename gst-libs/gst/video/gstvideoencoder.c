...
struct _GstVideoEncoderPrivate
{
  ...
  GstClockTime last_keyframe_time;
  GstClockTime keyframe_rate_limit;   /* Rate limit keyframes generated by force-key-unit events */
...
gst_video_encoder_reset (GstVideoEncoder * encoder, gboolean hard)
{
  ...
  priv->last_keyframe_time = GST_CLOCK_TIME_NONE;
...
void
gst_video_encoder_set_keyframe_rate_limit (GstVideoEncoder * video_encoder,
    GstClockTime limit)
{
  video_encoder->priv->keyframe_rate_limit = limit;
}
...
static void
gst_video_encoder_handle_key_unit_event (GstVideoEncoder * encoder,
    GstClockTime running_time, gboolean all_headers, guint count)
{
  gboolean ignore = FALSE;

  if (encoder->priv->keyframe_rate_limit &&
      running_time == GST_CLOCK_TIME_NONE &&
      GST_CLOCK_TIME_IS_VALID (encoder->priv->last_keyframe_time)) {
    if (encoder->priv->force_key_unit) {
      /* only queue up one event to avoid bursts of keyframes */
      ignore = TRUE;
    } else {
      /* ignore if event is close to previous keyframe */
      GstClock * clock = gst_element_get_clock (GST_ELEMENT_CAST (encoder));
      if (clock) {
        GstClockTime now = gst_clock_get_time (clock);
        GstClockTimeDiff diff = GST_CLOCK_DIFF (
            encoder->priv->last_keyframe_time, now);
        gst_object_unref (clock);

        if (diff < encoder->priv->keyframe_rate_limit) {
          ignore = TRUE;
          GST_INFO_OBJECT (encoder, "ignore force-key-unit event:"
              " last-key-unit %" GST_TIME_FORMAT " now %" GST_TIME_FORMAT
              " limit %" GST_TIME_FORMAT,
              GST_TIME_ARGS (GST_CLOCK_DIFF (
                      GST_ELEMENT_CAST(encoder)->base_time,
                      encoder->priv->last_keyframe_time)),
              GST_TIME_ARGS (GST_CLOCK_DIFF (
                      GST_ELEMENT_CAST(encoder)->base_time, now)),
              GST_TIME_ARGS (encoder->priv->keyframe_rate_limit));
        }
      }
    }
  }
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      if (gst_video_event_is_force_key_unit (event)) {
        ...
        if (gst_video_event_parse_downstream_force_key_unit (event,
          ...
          gst_video_encoder_handle_key_unit_event (encoder, running_time,
              all_headers, count);
        }
        gst_event_unref (event);
        event = NULL;
    ...
    GstVideoCodecFrame * frame)
{
  ...
  if (GST_VIDEO_CODEC_FRAME_IS_SYNC_POINT (frame)) {
    ...
    if (priv->keyframe_rate_limit && priv->time > 0) {
      GstClock * clock = gst_element_get_clock (GST_ELEMENT_CAST (encoder));
      if (clock) {
        priv->last_keyframe_time = gst_clock_get_time (clock);
        gst_object_unref (clock);
        GST_INFO_OBJECT (encoder, "frame is keyframe: %" GST_TIME_FORMAT,
            GST_TIME_ARGS (GST_CLOCK_DIFF (
                    GST_ELEMENT_CAST (encoder)->base_time,
                    priv->last_keyframe_time)));
      }
    }
  } else {
