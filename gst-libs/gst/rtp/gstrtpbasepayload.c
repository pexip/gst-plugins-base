    ...
    for (i = 0; i < n_fields; i++) {
      ...
      if (g_str_has_prefix (field_name, "extmap-")) {
        ...
        for (j = 0; j < header_exts->len; j++) {
          ...
          if (gst_rtp_header_extension_get_id (ext) == ext_id) {
            if (g_strcmp0 (uri, gst_rtp_header_extension_get_uri (ext)) == 0) {
              /* still matching, we're good. FIXME: may still need to change
               * attributes though */
              break;
            } else {
        ...
        if (!ext) {
          ...
          if (!ext) {
            /* try to create one if request failed */
            gst_rtp_header_extension_create_from_uri (uri);
            if (!ext) {
              GST_WARNING_OBJECT (payload, "could not retrieve header "
                  "extension for id %" G_GUINT64_FORMAT " and uri %s", ext_id,
                  uri);
              res = FALSE;
              goto ext_out;
            }
            ...
            /* FIXME: convert from possible extmap attributes to setting
             * extension options? */
          }
